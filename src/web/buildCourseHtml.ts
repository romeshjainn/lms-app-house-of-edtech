export function buildCourseHtml(): string {
  return `<!DOCTYPE html>
<html lang="en">
<head>
  <meta charset="UTF-8" />
  ...
  <style>
    *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    #loader {
      ...
      min-height: 100vh; gap: 12px; color: var(--muted);
    }
    .spinner {
    ...
    .hero { width: 100%; aspect-ratio: 16/9; object-fit: cover; background: var(--border); }
    .meta { padding: 20px 16px 8px; background: var(--white); }
    .category-tag {
    ...
    .course-title { font-size: 20px; font-weight: 700; line-height: 1.3; }
    ...
    .section { background: var(--white); margin-top: 10px; padding: 16px; }
    .section-heading { font-size: 15px; font-weight: 700; margin-bottom: 12px; }
    .instructor-card { display: flex; gap: 12px; align-items: flex-start; }
    .instructor-avatar {
    ...
    .btn-complete:disabled { background: var(--success); cursor: default; }
  </style>
</head>
<body>
  ...
  <script>
    var FAKE_CURRICULUM = [
      ...
      { title: 'Module 2 \u2013 Core Concepts', lessons: [
    ...
    window.__initCourse__ = function(course, isAlreadyCompleted, isGuest, isEnrolled) {
      ...
      if (course.instructor && course.instructor.location)
        meta.push('\uD83D\uDCCD ' + course.instructor.location);
      if (course.instructor && course.instructor.email)
        meta.push('\u2709\uFE0F ' + course.instructor.email);
      document.getElementById('instructor-meta').textContent = meta.join('  \u00B7  ');

      document.getElementById('description').textContent =
        course.description || 'No description available.';

      var container = document.getElementById('curriculum');
      FAKE_CURRICULUM.forEach(function(mod) {
